<?php if(count($data['bills']) > 0) { ?>
  <?php foreach($data['bills'] as $bill) { ?>
    <div class="card">
      <div class="card-body">
        <h5 class="card-title">قبض <?php echo $bill['bill_type']; ?></h5>
        <p>حساب این واحد: <span><?php echo $bill['bill_price']; ?></span>تومان</p>
        <p>وضعیت قبض : <span><?php echo $bill['bill_status']; ?></span></p>
        <p>شماره واحد ساختمان:<span><?php echo $bill['number']; ?></span></p>
        <p>نام  شخص:<span><?php echo $bill['person_name']; ?></span></p>
        <p>شماره تماس:<span><?php echo $bill['person_phone']; ?></span></p>
      </div>
